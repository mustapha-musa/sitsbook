...
<!---
PEDRO: 13) O trecho abaixo apresenta tibble, termo que ja foi usado em varios outros capitulos. acho que poderia mover parte deste paragrafo mais para o comeco do livro. O restante deste capitulo fala bastante de "sits tibble", nao sei se e' a melhor estrategia uma vez que varios outros objetos no
pacote tambem sao "sits tibbles".
-->
...
<!---
PEDRO: 14) Nao esta claro o termo instance acima. Seria "time instances"?
-->
...
```
<!---
PEDRO: 15) No exemplo acima, o ultimo argumento sits_get_data e' seguido de fecha parenteses. tem outros exemplos que o fecha parenteses fica sozinho na linha seguints. seria bom manter consistencia. prefiro deste jeito, pois economiza espaco.
-->
...
The literature on satellite image time series has several applications of filtering to correct or smooth vegetation index data. The package supports the well-known Savitzky–Golay (`sits_sgolay()`) and Whittaker (`sits_whittaker()`) filters. In an evaluation of MERIS NDVI time series filtering for estimating phenological parameters in India, @Atkinson2012 found that the Whittaker filter provides good results. 
<!---
PEDRO: 16) gostaria de confirmar se MERIS NDVI poderia ser apenas NDVI. O termo MERIS so' aparece uma vez em todo o livro.
-->
@Zhou2016 found that the Savitzky-Golay filter is suitable for reconstructing tropical evergreen broadleaf forests.
...
<!---
PEDRO: 17) esta ultima frase nao ficou boa, a ainda usa o termo "users". remover?
-->
...
<!---
PEDRO: 18) pessoalmente nao gosto de aspas nestas situacoes. poderia substituir por italico? tem outras situacoes assim no texto
-->

The following example shows the effect of the Whittaker filter on a point extracted from the MOD13Q1 product, ranging from 2000-02-18 to 2018-01-01. The `lambda` parameter controls the smoothing of the filter. By default, it is set to 0.5, a small value. 
<!---
PEDRO: 19) as duas frases acima poderiam estar no paragrafo anterior
-->
For illustrative purposes, we show the effect of a larger smoothing parameter.
\newpage
