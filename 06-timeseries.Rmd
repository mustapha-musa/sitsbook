...
To get time series in `sits`, one has to first create a regular data cube and then provide a set of locations inside the data cube using `sits_get_data()`. This function uses two mandatory parameters: `cube` and `samples`. The `cube` parameter states to specify what are the locations whose values will be retrieved. The `samples` parameter accepts the following data types: 
