...
```{r, out.width = "80%", tidy="styler", fig.align="center", fig.cap="Most relevant variables of trained random forest model."}
...
```{r seval, fig.align="center", out.width="90%", fig.cap="Confusion by cluster for the balanced data set"}
# Show the result
plot(som_eval) 
```
...
```{r, tidy="styler",out.width = "90%", fig.align="center", fig.cap="Most important variables in random forest model."}
...
```{r, tidy="styler",out.width = "90%", fig.align="center", fig.cap="Classification of time series using random forest."}
...
```{r tcnn, echo = FALSE, fig.align="center", out.width = "100%", fig.cap="Structure of tempCNN architecture (source: Pelletier et al.(2019))"}
...
```{r rnet, echo = FALSE, out.width = "100%", fig.align="center", fig.cap="Structure of ResNet architecture (source: Wang et al.(2017))."}
...
<!---
PEDRO: 47) O acronimo MLP e' definido duas vezes neste Capitulo, no paragrafo acima e na linha 179
GILBERTO: Resolvido.
-->
...
GILBERTO: Boa observação. Ajustei tudo para usar "land classification" e "land class". Coloquei um novo parágrafo na Introdução que fala de "land use" e "land cover".  
-->
...
<!---
PEDRO: 31) Existem duas ocorrencias de "product" neste capitulo. Usar classification?
GILBERTO: resolvido.
-->
...
We assume that the likelihood $x_{i,k} | \mu_{i,k}$ follows a normal distribution, $N(\mu_{i,k}, \sigma^2_{k})$, with mean $\mu_{i,k}$ and variance $\sigma^2_{k}$. 
<!---
PEDRO: 35) "with parameters $\mu_{i,k}$ and $\sigma^2_{k}$" poderia ser removido, visto que ja esta explicito na formula.
GILBERTO: mean and variance fica melhor.
-->

The variance $\sigma^2_{k}$ is a hyperparameter that controls the smoothness of the resulting estimate. Therefore
<!---
...
The resulting Excel file can be opened with R or using spreadsheet programs. Figure \@ref(fig:xls) shows a printout of what is read by Excel. 
<!---
PEDRO: 40) Todo o texto do livro faz referencias a figuras "below", posso trocar pelo numero da figura?
GILBERTO: Tentei arrumar todas as referencias das figuras de forma mais generica. Veja como ficou.
-->
...
```{r xls, echo = FALSE, fig.align="center", out.width = "90%", out.height = "90%", fig.cap= "Result of 5-fold cross-validation of Mato Grosso data using LightTAE"}
...
```{r al, echo = FALSE, out.width = "100%", fig.align="center", fig.cap="Active learning approach (source: Crawford et al., 2013)"}
...
$$
<!---
PEDRO: 43) Pode remover os parenteses externos da formula?
GILBERTO: Pode.
-->
...
GILBERTO: Removido.
-->
This can be especially useful when working with complex or uncertain data. By combining the predictions of multiple models, users can identify which features or factors are most important for making accurate predictions. When using ensemble methods, choosing diverse models with different sources of error is important to ensure that the ensemble predictions are more accurate and robust.

<!---
PEDRO: 45) Este paragrafo acima parece mais um fechamento do capitulo.
GILBERTO: De acordo. Colocado no fim do capítulo.
-->
...
```{r, tidy = "styler", out.width = "100%", fig.align="center", fig.cap= "Land classification in Rondonia using a random forest algorithm."}
...
The color table can be extended or adjusted by accessing and modifying the default color table, using `sits_colors()` to retrieve the table.
<!---
PEDRO: 46) sits_colors_set() nao e' mostrado no exemplo a seguir, somente depois do proximo paragrafo
GILBERTO: Texto modificado (rever)
-->
...
The original default `sits` colour table can be restored using `sits_colors_reset()`. 
