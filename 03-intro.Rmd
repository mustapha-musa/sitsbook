...
GILBERTO: Sim.
P: Alterei tudo, porem quando o nome da funcao esta no comeco de uma frase eu preferi
manter. por exemplo: "The `sits_train()` function provides [...]". Posso manter assim
ou atualizar para "`sits_train()` provides [...]", porque o paragrafo iria comecar
com uma letra minuscula. De acordo?
-->
...
After removing outliers using local smoothing, the labeled classification map can be obtained using `sits_label_classification()`. This function assigns each pixel to the class with the highest probability.
\newpage
