...
<!---
PEDRO: 1) a parte do "assigned to each pixel by time series classification" na verdade vem do time-first, mas nao foi falado nada no paragrafo anterior sobre isto
GILBERTO: veja o novo texto.
-->
...
<!---
PEDRO: 2) "classifier, which classifies" -> classifier, which assigns classes/labels?
GILBERTO: "algorithm". 
-->
...
<!---
PEDRO: 3) "extracted samples" e' a mesma coisa de "Locations with known labels" (mais acima), certo? Talvez seja melhor usar o mesmo termo.
GILBERTO: "time series samples". 
-->
...
<!---
PEDRO: 4) posso colocar links para os capitulos?
GILBERTO: Pode, por favor.
-->
...
```
<!---
PEDRO: 5) o separador aqui é sempre "_", certo? O que acontece com X1, X2? Viram atributos ou sao ignorados?
Gilberto: Veja a seguir.
-->

The aim of the `parse_info` parameter is to extract `tile`, `band` and `date` information from the file name. Given the large variation in image file names generated by different produces, it includes designators such as `X1` and `X2`; these are place holders for parts of the file name that is not relevant to the `sits_cube` function. 
...
<!---
PEDRO: 6) aqui o termo "table" e' usado como sinonimo de tibble, mas tibble ainda nao foi definido (so' veja que na saida do codigo abaixo aparece o termo tibble).
GILBERTO: Favor ver novo texto.
-->

To handle time series information, `sits` uses a `tibble`. Tibbles are extensions of the `data.frame` tabular data structures provided by the `tidyverse` set of packages. The example below shows a table with 1,218 time series obtained from MODIS MOD13Q1 images. Each series has four attributes: two bands ("NIR" and "MIR") and two indexes ("NDVI" and "EVI"). This data set is available in package `sitsdata`.
...
<!---
PEDRO: 7) tem uma regra em documentacoes de pacotes que diz que se o código tem () é porque é uma funcao, e portanto nao deveria ser seguido de "function". Essa regra sera seguida neste livro?
GILBERTO: Sim.
-->
...
<!---
PEDRO: 8) veja que aqui ja esta chamando de tibble, nao mais de table
GILBERTO: Sim, foi corrigido no inicio do capítulo.
-->
...
<!---
PEDRO: 9) nao definiu EO antes, nem big. EO e' definido em 04-datacubes.Rmd e depois so' e' usado em 09-rasterclassification.Rmd. big e' usado em varios lugares.
GILBERTO: Vamos colocar por extenso.
-->

When working with big Earth observation data, 
