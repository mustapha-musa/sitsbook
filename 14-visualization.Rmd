...
In examples provided in the book, the color legend is taken from a predefined color table provided by `sits`. This default color table is displayed using `sits_colors_show()`. This color definition file assigns colors to 99 class names, including the IPCC and IGBP land use classes. 
...
The color table can be extended or adjusted by accessing and modifying the default color table, using `sits_colors()` to retrieve the table and `sits_colors_set()` to update the table, as shown in the example below.
<!---
