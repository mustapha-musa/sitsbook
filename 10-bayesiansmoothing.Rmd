...
To be reliable, local statistics should only include pixels likely to belong to a single class. Windows centred on border pixels contain only some pixels belonging to the same class as the central pixel; the others belong to a different class. Consider a window of size $7 \times 7$ around a pixel in the probability map of class "Forest". It will contain the central pixel and 80 neighbours.
<!---
